...
class TestBackupRollbackIntegration(unittest.TestCase):
    ...
    def test_deploy_moonraker_rollback_on_failed_verification(
        ...
        """Verify deploy_moonraker creates backups, fails verification, and triggers config rollback."""
        ...
        mock_download.return_value = (True, b"old_printer_cfg_data")
        
        # 2. Upload of new files returns success
        ...
        # Verify old printer.cfg data was uploaded during rollback
        # Must be called twice: 1st for the new file deployment, 2nd for rollback restore
        self.assertEqual(mock_upload.call_count, 2)
        
        # Check that the second call wrote the backup content
        # Because NamedTemporaryFile content is read, check that upload_printer_cfg was invoked with filename="printer.cfg"
        self.assertEqual(mock_upload.call_args[1]["filename"], "printer.cfg")
        
        # Verify rollback prints were generated
        ...
        self.assertTrue(any("Rollback complete" in msg for msg in printed))
    ...
    def test_deploy_ssh_rollback_on_failed_verification(
        ...
        # Remote printer.cfg exists (stat does not raise)
        mock_sftp.stat.return_value = MagicMock(st_size=123)
        
        # Mock SSH executes for systemctl
        mock_stdout_active = MagicMock()
        mock_stdout_active.read.return_value = b"inactive\n"
        
        mock_stdout_failed = MagicMock()
        mock_stdout_failed.read.return_value = b"failed\n"
        
        mock_stdout_journal = MagicMock()
        mock_stdout_journal.read.return_value = b"Traceback (most recent call last):\n  File 'klipper.py' ...\n"
        
        def mock_exec_command(cmd, *args, **kwargs):
            if "is-active" in cmd:
                return (MagicMock(), mock_stdout_active, MagicMock())
            elif "is-failed" in cmd:
                return (MagicMock(), mock_stdout_failed, MagicMock())
            elif "journalctl" in cmd:
                return (MagicMock(), mock_stdout_journal, MagicMock())
            else:
        ...
        # Verify stat was called to check existence, and rename was called to backup
        mock_sftp.rename.assert_any_call("/home/pi/printer_data/printer.cfg", "/home/pi/printer_data/printer.cfg.bak")
        
        # Verify rollback restored the backup
        mock_sftp.rename.assert_any_call("/home/pi/printer_data/printer.cfg.bak", "/home/pi/printer_data/printer.cfg")
        
        # Verify journalctl logs and rollback actions printed
        printed = [call[0][0] for call in mock_print.call_args_list]
        self.assertTrue(any("Verification FAILED" in msg for msg in printed))
        self.assertTrue(any("Initiating automatic rollback" in msg for msg in printed))
        self.assertTrue(any("Traceback (most recent call last)" in msg for msg in printed))
